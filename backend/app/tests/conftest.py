...
async def db_session():
    ...
    Tests must take this rather than importing TestAsyncSessionLocal
    directly: pytest loads conftest under its own module name, so a plain
    import re-executes this file and builds a second, empty database.
    """
    async with TestAsyncSessionLocal() as session:
        yield session
